...
const REGIONAL_QUOTE_FIXTURES: RegionalQuoteFixture[] = [
  ...
  {
    name: 'no zone lists the country and there is no rest of world: the store fee and threshold, as without zones',
    input: {
  ...
  {
    name: 'a zone with no active rate ships at the store fee, with its own threshold and cash-on-delivery rule',
    input: {
      config: {
        zones: [PARITY_ZONES[0]],
        rates: [
          PARITY_RATES[3],
          {
            id: 'r-eu-off',
  ...
  {
    name: 'a zone with no active rate: its own threshold makes the store fee free, not the store threshold',
    input: {
      config: { zones: [PARITY_ZONES[0]], rates: [], taxRates: PARITY_TAX_RATES },
      store: PARITY_STORE,
      items: [{ ...PARITY_TOOL_LINE, quantity: 3 }],
      destination: GERMANY,
      fulfillment: 'delivery',
  ...
  {
    name: 'a rest-of-world zone with no rate ships at the store fee and ignores the store threshold',
    input: {
      config: { zones: [PARITY_ZONES[1]], rates: [], taxRates: [] },
      store: PARITY_STORE,
      items: [{ id: 'd', price: 150, quantity: 1 }],
      destination: BRAZIL,
      fulfillment: 'delivery',
  ...
  {
    name: 'a zone marked do-not-ship refuses delivery to its countries',
    input: {
      config: {
        zones: [
          {
  ...
  {
    name: 'a rest-of-world zone marked do-not-ship means "only my zones": an unlisted country is refused',
    input: {
      config: {
        zones: [PARITY_ZONES[0], { ...PARITY_ZONES[1], do_not_ship: true }],
        rates: PARITY_RATES,
        taxRates: PARITY_TAX_RATES,
      },
      store: PARITY_STORE,
  ...
  {
    name: 'a rest-of-world zone marked do-not-ship: a listed country still ships with its own zone',
    input: {
      config: {
        zones: [PARITY_ZONES[0], { ...PARITY_ZONES[1], do_not_ship: true }],
        rates: PARITY_RATES,
  ...
  {
    name: 'store pickup is never refused, even from a do-not-ship zone',
    input: {
      config: {
        zones: [
          {
