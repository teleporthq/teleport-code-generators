...
describe('Next generator with a Scroll Scene element', () => {
  ...
  it('ships the reveal and count lanes in the runtime, with the count rule', async () => {
    ...
    expect(component?.content).toContain('function passedScenePoints')
    expect(component?.content).toContain("'tq-scene-point-passed'")
    expect(component?.content).toContain("'data-scene-point'")
