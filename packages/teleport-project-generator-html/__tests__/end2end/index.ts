...
describe('Unwinds the slot inside the component when used in page', () => {
  it('runs without crashing', async () => {
    ...
    expect(indexFile?.content).toContain(`homeimage2`)
    expect(indexFile?.content).toContain(`homeimage3`)
    expect(cssFile).toBeDefined()
    expect(cssFile?.content).toContain(`.homeimage2`)
    expect(cssFile?.content).toContain(`.homeimage3`)
  })
})
