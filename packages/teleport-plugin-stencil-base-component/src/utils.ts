...
import { capitalize, camelCaseToDashCase } from '@teleporthq/teleport-shared/lib/utils/string-utils'
...
export const createClassDeclaration = (
  ...
  switch (nodeType) {
    case 'static':
      renderBody = typeof jsxTagTree === 'string' && types.stringLiteral(jsxTagTree)
      break
    case 'dynamic':
    case 'conditional':
      renderBody =
      ...
      break
    default:
      renderBody = createReturnExpressionSyntax(stateDefinitions, jsxTagTree as types.JSXElement)
      break
  }

  const decoratorParam = objectToObjectExpression({
  ...
  const propDeclaration = Object.keys(propDefinitions).map((propKey) =>
    t.classProperty(
      ...
      t.stringLiteral(propDefinitions[propKey].defaultValue.toString()),
  ...
  const classBody = t.classBody([
    ...propDeclaration,
    types.classMethod('method', t.identifier('render'), [], renderBody),
  ])

  const classDeclaration = t.classDeclaration(t.identifier(name), null, classBody, [
    t.decorator(t.callExpression(t.identifier('Component'), [decoratorParam])),
  ])
